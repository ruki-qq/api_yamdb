...
class Test01UserAPI:
    ...
    def test_02_users_username_not_authenticated(self, client, admin):
        ...
        assert response.status_code == HTTPStatus.UNAUTHORIZED, (
            f'Проверьте, что GET-запрос `{self.USERS_URL}{{username}}/` без'
    ...
    def test_04_02_users_get_search(
        ...
        assert len(reponse_json['results']) == users_count, (
            'Проверьте, что GET-запрос к'
            f' `{self.USERS_URL}?search={{username}}` возвращает данные только'
            ' тех пользователей, `username` которых удовлетворяет условию'
            ' поиска.'
        )
        admin_as_dict = {
            'username': admin.username,
